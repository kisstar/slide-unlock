 ...
 * @function
 * @name bindEvents
 * @param {string | Element} element 绑定事件的选择器或元素
 * @param {string} events 一个以空格分开的字符串，指定了要绑定的事件
 * @param {function} handler 事件监听器
 * @param {boolean | object} options 指定有关 handler 属性的可选参数对象
 * @returns {void}
 */
export const bindEvents = <K extends keyof ElementEventMap>(
  element: string | Element,
  events: string,
  handler: (this: Document, ev: ElementEventMap[K]) => any,
 ...
 * @returns {void}
 */
export const unbindEvents = <K extends keyof ElementEventMap>(
  element: string | Element,
  events: string,
  handler: (this: Document, ev: ElementEventMap[K]) => any,
  options: boolean | AddEventListenerOptions = false,
